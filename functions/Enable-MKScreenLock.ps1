function Enable-MKScreenLock {
	...
	$params = @{
		...
		ImageOpacity = 2
		Opacity = 5
		MessageContent = "   PowerShell Saturday: `nData Piracy : Data Privacy"
		Duration = "00:12:00"
	}
	...
	param (
		...
		[Parameter()]
		[ValidateSet(
			'black', 'blue', 'brown', 'cyan', 'green', 'gray',
		...
		foreach ($screen in $screens) {
			...
			try {
				...
				if ($screen.Primary) {
					...
					$contentBorder.Height = $screen.Bounds.Height * 0.3
					...
					$window.add_Loaded({
							$messageTextBlock.FontSize = $window.ActualWidth * 0.05
